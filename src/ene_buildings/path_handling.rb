...
# Internal: Mixin module to handle path creation and reposition.
# See initialize for a list of class instance variable used by these methods
# and what they do.
module PathHandling
  ...
  COLOR_ARROW             = Sketchup::Color.new 255, 255,   0
  ...
  # Internal: Sets some class variables, either to given values or defaults.
  # Typically called from initialize in each tool.
  #
  ...
  # end_angles       - Array containing angle for each building end or nil when
  #                    end angles shouldn't be enabled.
  ...
  def getMenu(menu, flags, x, y, view)
    ...
    unless @back_along_path.nil?
      submenu = menu.add_submenu("Side to Follow Path")
      item = submenu.add_item("Front") { @back_along_path = false; view.invalidate }
      submenu.set_validation_proc(item) { !@back_along_path ? MF_CHECKED : MF_UNCHECKED }
      item = submenu.add_item("Back") { @back_along_path = true; view.invalidate }
      submenu.set_validation_proc(item) { @back_along_path ? MF_CHECKED : MF_UNCHECKED }
    end
