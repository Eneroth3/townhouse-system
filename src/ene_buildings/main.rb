...
module EneBuildings
  ...
  require "json"
  require "rubygems"
  Sketchup.require File.join(PLUGIN_DIR, "gem_installer")
  
  unless Gem::Version.new(Sketchup.version) >= Gem::Version.new(REQUIRED_SU_VERSION)
  ...
  # Only run rest of code once required Gems are available.
  gem_installer(required_gems) do
    ...
    def self.attr_dict_to_hash(entity, dict_name)

      h = {}
      ad = entity.attribute_dictionary(dict_name)
      return h unless ad
      ad.each_pair { |k, v| h[k] = v }
